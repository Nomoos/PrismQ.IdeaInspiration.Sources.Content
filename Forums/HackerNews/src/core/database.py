"""Database wrapper for HackerNews source.

This module provides backward compatibility with the Database class pattern
...
class Database:
    """Manages database operations for HackerNews post collection.
    
    This class wraps db_utils for consistent database interface.
    ...
    def get_idea(self, source: str, source_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific HackerNews post by source and source_id.
        ...
        with db_utils.get_connection(self.database_url) as conn:
            result = conn.execute(
                text("SELECT * FROM HackerNewsSource WHERE source = :source AND source_id = :source_id"),
