"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Shorts.TwitchClips."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        self.twitch_client_id = self._get_or_prompt(
            "TWITCH_CLIENT_ID",
        ...
        self.twitch_client_secret = self._get_or_prompt(
        ...
        # Twitch Trending configuration
        twitch_trending_max_clips = self._get_or_prompt(
            "TWITCH_TRENDING_MAX_CLIPS",
            "Maximum clips to scrape from trending",
            "10",
            required=False
        )
        self.twitch_trending_max_clips = int(twitch_trending_max_clips) if twitch_trending_max_clips else 10
        
        # Twitch Game configuration
        self.twitch_game_name = self._get_or_prompt(
            "TWITCH_GAME_NAME",
            "Twitch game name (optional)",
            "",
            required=False
        )
        
        twitch_game_max_clips = self._get_or_prompt(
            "TWITCH_GAME_MAX_CLIPS",
            "Maximum clips to scrape per game",
            "10",
            required=False
        )
        self.twitch_game_max_clips = int(twitch_game_max_clips) if twitch_game_max_clips else 10
        
        # Twitch Streamer configuration
        self.twitch_streamer_name = self._get_or_prompt(
            "TWITCH_STREAMER_NAME",
            "Twitch streamer name (optional)",
            "",
            required=False
        )
        
        twitch_streamer_max_clips = self._get_or_prompt(
            "TWITCH_STREAMER_MAX_CLIPS",
            "Maximum clips to scrape per streamer",
            "10",
            required=False
        )
        self.twitch_streamer_max_clips = int(twitch_streamer_max_clips) if twitch_streamer_max_clips else 10
