"""Database utility functions for TikTok source using DATABASE_URL.
...
def get_engine(database_url: str):
    ...
    Args:
        database_url: Database URL (e.g., sqlite:///tiktok.s3db)
...
def insert_idea(database_url: str, source: str, source_id: str, title: str,
    ...
    with get_connection(database_url) as conn:
        ...
        result = conn.execute(
            text("SELECT id FROM TikTokSource WHERE source = :source AND source_id = :source_id"),
...
def get_idea(database_url: str, source: str, source_id: str) -> Optional[Dict[str, Any]]:
    ...
    with get_connection(database_url) as conn:
        result = conn.execute(
            text("SELECT * FROM TikTokSource WHERE source = :source AND source_id = :source_id"),
            {"source": source, "source_id": source_id}
        )
        row = result.fetchone()
...
def get_all_ideas(database_url: str, source: Optional[str] = None, 
    ...
    """
    with get_connection(database_url) as conn:
        query = "SELECT * FROM TikTokSource WHERE 1=1"
        params = {}
...
def mark_processed(database_url: str, source: str, source_id: str) -> bool:
    ...
    with get_connection(database_url) as conn:
        result = conn.execute(
            text("""
                UPDATE TikTokSource 
                SET processed = 1, updated_at = :updated_at
                WHERE source = :source AND source_id = :source_id
            """),
            {
                "updated_at": utc_now(),
...
def get_statistics(database_url: str) -> Dict[str, Any]:
    ...
    with get_connection(database_url) as conn:
        # Total count
        total = conn.execute(text("SELECT COUNT(*) as count FROM TikTokSource")).fetchone()
        
        # Count by source
        by_source = conn.execute(
            text("SELECT source, COUNT(*) as count FROM TikTokSource GROUP BY source")
        ).fetchall()
        
        # Processed vs unprocessed
        processed_count = conn.execute(
            text("SELECT COUNT(*) as count FROM TikTokSource WHERE processed = 1")
        ).fetchone()
        
        unprocessed_count = conn.execute(
            text("SELECT COUNT(*) as count FROM TikTokSource WHERE processed = 0")
        ).fetchone()
        
        return {
            "total": total[0] if total else 0,
            "by_source": {row[0]: row[1] for row in by_source},
            "processed": processed_count[0] if processed_count else 0,
            "unprocessed": unprocessed_count[0] if unprocessed_count else 0
        }
