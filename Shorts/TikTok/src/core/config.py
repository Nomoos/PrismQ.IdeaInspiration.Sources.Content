"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Shorts.TikTok."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            ...
            f"sqlite:///{Path(self.working_directory) / 'tiktok.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "tiktok.s3db")
        
        # TikTok Trending configuration
        tiktok_trending_max = self._get_or_prompt(
            "TIKTOK_TRENDING_MAX",
            "Maximum videos to scrape from trending",
            "10",
            required=False
        )
        self.tiktok_trending_max = int(tiktok_trending_max) if tiktok_trending_max else 10
        
        # TikTok Hashtag configuration
        self.tiktok_hashtag = self._get_or_prompt(
            "TIKTOK_HASHTAG",
            "Default TikTok hashtag to scrape (optional)",
            "",
            required=False
        )
        
        tiktok_hashtag_max = self._get_or_prompt(
            "TIKTOK_HASHTAG_MAX",
            "Maximum videos to scrape per hashtag",
            "10",
            required=False
        )
        self.tiktok_hashtag_max = int(tiktok_hashtag_max) if tiktok_hashtag_max else 10
        
        # TikTok Creator configuration
        self.tiktok_creator_username = self._get_or_prompt(
            "TIKTOK_CREATOR_USERNAME",
            "TikTok creator username to scrape (optional)",
            "",
            required=False
        )
        
        tiktok_creator_max = self._get_or_prompt(
            "TIKTOK_CREATOR_MAX",
            "Maximum videos to scrape from creator",
            "10",
            required=False
        )
        self.tiktok_creator_max = int(tiktok_creator_max) if tiktok_creator_max else 10
        
        # Rate limiting configuration
        tiktok_rate_limit_delay = self._get_or_prompt(
            "TIKTOK_RATE_LIMIT_DELAY",
            "Delay between requests in seconds",
            "2",
            required=False
        )
        self.tiktok_rate_limit_delay = float(tiktok_rate_limit_delay) if tiktok_rate_limit_delay else 2.0
