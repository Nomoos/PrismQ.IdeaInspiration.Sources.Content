"""Database utility functions for Instagram Reels source.

This module provides database access using SQLAlchemy Core
with connection configured via database URL.
"""
...
def init_database(database_url: str):
    ...
    # Define InstagramReelsSource table
    Table(
...
def insert_idea(database_url: str, source: str, source_id: str, title: str,
    ...
    with get_connection(database_url) as conn:
        ...
        result = conn.execute(
            text("SELECT id FROM InstagramReelsSource WHERE source = :source AND source_id = :source_id"),
        ...
        if existing:
            ...
            conn.execute(
                text("""
                    INSERT INTO InstagramReelsSource
...
def get_unprocessed_records(database_url: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    """
    with get_connection(database_url) as conn:
        query = "SELECT * FROM InstagramReelsSource WHERE processed = 0 OR processed IS NULL"
        if limit:
            query += f" LIMIT {limit}"
...
def mark_as_processed(database_url: str, record_id: int):
    ...
    with get_connection(database_url) as conn:
        conn.execute(
            text("UPDATE InstagramReelsSource SET processed = 1, updated_at = :updated_at WHERE id = :id"),
...
def get_all_ideas(database_url: str, limit: int = 20, order_by: str = "score") -> List[Dict[str, Any]]:
    ...
    with get_connection(database_url) as conn:
        ...
        valid_columns = ['id', 'score', 'created_at', 'updated_at', 'title']
        if order_by not in valid_columns:
            order_by = 'score'
        
        query = f"SELECT * FROM InstagramReelsSource ORDER BY {order_by} DESC LIMIT {limit}"
        result = conn.execute(text(query))
        columns = result.keys()
        return [dict(zip(columns, row)) for row in result.fetchall()]
...
def get_stats(database_url: str) -> Dict[str, Any]:
    ...
    with get_connection(database_url) as conn:
        # Total ideas
        result = conn.execute(text("SELECT COUNT(*) as count FROM InstagramReelsSource"))
        total = result.fetchone()[0]
        
        # Processed ideas
        result = conn.execute(text("SELECT COUNT(*) as count FROM InstagramReelsSource WHERE processed = 1"))
        processed = result.fetchone()[0]
        
        # Unprocessed ideas
        result = conn.execute(text("SELECT COUNT(*) as count FROM InstagramReelsSource WHERE processed = 0 OR processed IS NULL"))
        unprocessed = result.fetchone()[0]
        
        # Ideas by source
        result = conn.execute(text("SELECT source, COUNT(*) as count FROM InstagramReelsSource GROUP BY source"))
