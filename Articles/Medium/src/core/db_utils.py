...
def insert_idea(database_url: str, source: str, source_id: str, title: str,
    ...
    with get_connection(database_url) as conn:
        ...
        result = conn.execute(
            text("SELECT id FROM MediumSource WHERE source = :source AND source_id = :source_id"),
...
def get_unprocessed_records(database_url: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    """
    with get_connection(database_url) as conn:
        query = "SELECT * FROM MediumSource WHERE processed = 0 OR processed IS NULL"
        if limit:
            query += f" LIMIT {limit}"
...
def mark_as_processed(database_url: str, record_id: int):
    ...
    with get_connection(database_url) as conn:
        conn.execute(
            text("UPDATE MediumSource SET processed = 1, updated_at = :updated_at WHERE id = :id"),
...
def get_all_ideas(database_url: str, limit: int = 20, order_by: str = "score") -> List[Dict[str, Any]]:
    ...
    with get_connection(database_url) as conn:
        query = f"SELECT * FROM MediumSource ORDER BY {order_by} DESC LIMIT {limit}"
        result = conn.execute(text(query))
        columns = result.keys()
        return [dict(zip(columns, row)) for row in result.fetchall()]
...
def count_ideas(database_url: str) -> int:
    ...
    """
    with get_connection(database_url) as conn:
        result = conn.execute(text("SELECT COUNT(*) as count FROM MediumSource"))
...
def count_by_source(database_url: str, source: str) -> int:
    ...
    with get_connection(database_url) as conn:
        result = conn.execute(
            text("SELECT COUNT(*) as count FROM MediumSource WHERE source = :source"),
            {"source": source}
        )
        return result.fetchone()[0]
