"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Articles.Medium."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        # Medium Trending configuration
        medium_trending_max_articles = self._get_or_prompt(
            "MEDIUM_TRENDING_MAX_ARTICLES",
            "Maximum articles to scrape from trending",
            "10",
            required=False
        )
        self.medium_trending_max_articles = int(medium_trending_max_articles) if medium_trending_max_articles else 10
        
        # Medium Tag configuration
        medium_tag_max_articles = self._get_or_prompt(
            "MEDIUM_TAG_MAX_ARTICLES",
            "Maximum articles to scrape by tag",
            "10",
            required=False
        )
        self.medium_tag_max_articles = int(medium_tag_max_articles) if medium_tag_max_articles else 10
        
        # Medium Author configuration
        medium_author_max_articles = self._get_or_prompt(
            "MEDIUM_AUTHOR_MAX_ARTICLES",
            "Maximum articles to scrape by author",
            "10",
            required=False
        )
        self.medium_author_max_articles = int(medium_author_max_articles) if medium_author_max_articles else 10
        
        # Medium Publication configuration
        medium_publication_max_articles = self._get_or_prompt(
            "MEDIUM_PUBLICATION_MAX_ARTICLES",
            "Maximum articles to scrape from publication",
            "10",
            required=False
        )
        self.medium_publication_max_articles = int(medium_publication_max_articles) if medium_publication_max_articles else 10
        
        # Request configuration
        self.user_agent = self._get_or_prompt(
            "USER_AGENT",
            "User agent for web scraping (optional)",
