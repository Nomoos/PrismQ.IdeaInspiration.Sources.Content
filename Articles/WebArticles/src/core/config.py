"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Articles.WebArticleSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            ...
            f"sqlite:///{Path(self.working_directory) / 'web_articles.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "web_articles.s3db")
        
        # Web article scraping configuration
        web_article_max_articles = self._get_or_prompt(
            "WEB_ARTICLE_MAX_ARTICLES",
            "Maximum articles to scrape per source",
            "10",
            required=False
        )
        self.web_article_max_articles = int(web_article_max_articles) if web_article_max_articles else 10
        ...
        # Request timeout
        web_article_timeout = self._get_or_prompt(
            "WEB_ARTICLE_TIMEOUT",
            "Request timeout in seconds",
            "30",
            required=False
        )
        self.web_article_timeout = int(web_article_timeout) if web_article_timeout else 30
