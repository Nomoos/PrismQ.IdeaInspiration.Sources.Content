...
class Database:
    """Manages database operations for article collection.
    ...
    def insert_article(self, source: str, source_id: str, title: str,
                      description: Optional[str] = None, content: Optional[str] = None,
                      tags: Optional[str] = None, author: Optional[str] = None,
                      url: Optional[str] = None, published_at: Optional[Any] = None,
        ...
        return db_utils.insert_article(
            self.database_url, source, source_id, title,
            description, content, tags, author, url, published_at,
            score, score_dictionary
        )
    
    def get_article(self, source: str, source_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific article by source and source_id.
        ...
        with db_utils.get_connection(self.database_url) as conn:
            result = conn.execute(
                text("SELECT * FROM WebArticleSource WHERE source = :source AND source_id = :source_id"),
    ...
    def get_all_articles(self, limit: int = 20, order_by: str = "score") -> List[Dict[str, Any]]:
        ...
        """
        return db_utils.get_all_articles(self.database_url, limit, order_by)
    ...
    def count_articles(self) -> int:
        ...
        Returns:
            Total count
        """
        return db_utils.count_articles(self.database_url)
