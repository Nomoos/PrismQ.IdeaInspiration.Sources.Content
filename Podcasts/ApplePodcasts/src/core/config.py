"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Podcasts.ApplePodcasts."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        apple_podcasts_max_episodes = self._get_or_prompt(
            "APPLE_PODCASTS_MAX_EPISODES",
            "Maximum episodes to scrape per show",
            "10",
            required=False
        )
        self.apple_podcasts_max_episodes = int(apple_podcasts_max_episodes) if apple_podcasts_max_episodes else 10
        
        apple_podcasts_max_shows = self._get_or_prompt(
            "APPLE_PODCASTS_MAX_SHOWS",
            "Maximum shows to scrape from charts",
            "20",
            required=False
        )
        self.apple_podcasts_max_shows = int(apple_podcasts_max_shows) if apple_podcasts_max_shows else 20
        
        # Apple Podcasts region
        self.apple_podcasts_region = self._get_or_prompt(
            "APPLE_PODCASTS_REGION",
            "Apple Podcasts region code (e.g., us, gb, ca)",
            "us",
            required=False
        )
