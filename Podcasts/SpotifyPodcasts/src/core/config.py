"""Configuration management for PrismQ.IdeaInspiration.Sources.Content.Podcasts.SpotifyPodcasts."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            ...
            f"sqlite:///{Path(self.working_directory) / 'spotify_podcasts.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "spotify_podcasts.s3db")
        
        # Spotify API configuration
        self.spotify_client_id = self._get_or_prompt(
            "SPOTIFY_CLIENT_ID",
        ...
        self.spotify_client_secret = self._get_or_prompt(
        ...
        # Spotify scraping configuration
        spotify_trending_max_episodes = self._get_or_prompt(
            "SPOTIFY_TRENDING_MAX_EPISODES",
            "Maximum episodes to scrape from trending",
            "10",
            required=False
        )
        self.spotify_trending_max_episodes = int(spotify_trending_max_episodes) if spotify_trending_max_episodes else 10
        
        spotify_category_max_episodes = self._get_or_prompt(
            "SPOTIFY_CATEGORY_MAX_EPISODES",
            "Maximum episodes to scrape per category",
            "10",
            required=False
        )
        self.spotify_category_max_episodes = int(spotify_category_max_episodes) if spotify_category_max_episodes else 10
        
        spotify_show_max_episodes = self._get_or_prompt(
            "SPOTIFY_SHOW_MAX_EPISODES",
            "Maximum episodes to scrape per show",
            "10",
            required=False
        )
        self.spotify_show_max_episodes = int(spotify_show_max_episodes) if spotify_show_max_episodes else 10
